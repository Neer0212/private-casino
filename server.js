...
const { Hand } = require('pokersolver'); // Math engine for Poker
...
async function checkRoundEnd(tableId, getUserBalance, updateBalance) {
    ...
    if (!tablePlayers.some(p => p.status === 'playing') && tablePlayers.length > 0) {
        ...
        for (let player of tablePlayers) {
            if (player.status === 'stood') {
                ...
                if (winAmount > 0) {
                    try {
                        let bal = await getUserBalance(player.username);
                        await updateBalance(player.username, bal + winAmount);
                    } catch (err) {}
                }
            }
        }
...
// 🤥 BLUFF (CHEAT) ENGINE
// ==========================================
function broadcastBluffState(tableId) {
    let table = bluffRooms[tableId];
    if (!table) return;
    
    let players = activePlayers.filter(p => p.tableId === tableId && p.game === 'bluff');
    
    let currentTurnUser = "";
    if (players.length > 0 && table.stage === 'playing') {
        let activeOnly = players.filter(p => p.hand.length > 0);
        if (activeOnly.length > 0) {
            currentTurnUser = activeOnly[table.turnIndex % activeOnly.length].username;
        }
    }
    ...
    io.to(tableId).emit('bluffStateUpdate', {
        stage: table.stage,
...
io.on('connection', (socket) => {
    ...
    socket.on('hit', (tableId) => {
        ...
        if (player && player.status === 'playing') {
            ...
            if (player.score > 21) { player.status = 'bust'; socket.emit('gameStatus', 'BUST! You lose.'); broadcastTableState(tableId); checkRoundEnd(tableId, getUserBalance, updateBalance); }
        }
    });
    socket.on('stand', (tableId) => {
        let player = activePlayers.find(p => p.socketId === socket.id && p.game === 'blackjack');
        if (player && player.status === 'playing') { player.status = 'stood'; socket.emit('gameStatus', 'Waiting...'); broadcastTableState(tableId); checkRoundEnd(tableId, getUserBalance, updateBalance); }
    });
    ...
    socket.on('spinRoulette', async (tableId) => {
        ...
        for (let player of bettors) {
            ...
            if (winAmount > 0) {
                try {
                    let balance = await getUserBalance(player.username);
                    await updateBalance(player.username, balance + winAmount);
    ...
    socket.on('joinPoker', async (data) => {
        ...
        if (!pokerRooms[tableId]) pokerRooms[tableId] = { deck: createDeck(), communityCards: [], stage: 'waiting', pot: 0, highestBet: 0, turnIndex: 0 };
        try {
            let balance = await getUserBalance(data.username);
    ...
    socket.on('devAdvancePokerStage', async (tableId) => {
        ...
        if (table.stage === 'waiting' || table.stage === 'showdown') {
            table.deck = createDeck(); table.communityCards = []; table.pot = 0; table.highestBet = 0; table.turnIndex = 0;
            // Collect ₹50 Ante
            for(let p of players) {
                try {
                    ...
                    if(bal >= 50) {
                        ...
                        p.status = 'playing'; p.handDesc = ''; p.currentBet = 50; table.pot += 50;
                        io.to(p.socketId).emit('receiveHoleCards', p.hand);
                    } else { p.status = 'waiting'; }
                } catch(err){}
            }
            table.highestBet = 50; table.stage = 'preflop';
            io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `🃏 New Hand! ₹50 Ante collected.` });

        } else if (table.stage === 'preflop') {
            table.deck.pop(); table.communityCards.push(table.deck.pop(), table.deck.pop(), table.deck.pop()); table.stage = 'flop'; resetRoundBets(tableId, players); io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `🃏 The Flop is dealt.` });
        } else if (table.stage === 'flop') {
            table.deck.pop(); table.communityCards.push(table.deck.pop()); table.stage = 'turn'; resetRoundBets(tableId, players); io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `🃏 The Turn is dealt.` });
        } else if (table.stage === 'turn') {
            table.deck.pop(); table.communityCards.push(table.deck.pop()); table.stage = 'river'; resetRoundBets(tableId, players); io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `🃏 The River is dealt.` });
            ...
            players.forEach(p => {
                if (p.status === 'playing') {
                    ...
                    solved.player = p; p.handDesc = solved.descr; solvedHands.push(solved);
                }
            });
    ...
    // --- 🤥 BLUFF (CHEAT) LISTENERS ---
    socket.on('joinBluff', (data) => {
        const tableId = (data.tableId && data.tableId.trim() !== "") ? data.tableId.trim() : "Bluff-1";
        socket.join(tableId);
    ...
    socket.on('devStartBluff', (tableId) => {
        let table = bluffRooms[tableId];
        let players = activePlayers.filter(p => p.tableId === tableId && p.game === 'bluff');
        if (!table || players.length < 2) return io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `❌ Need at least 2 players to start.` });
        ...
        players.forEach(p => io.to(p.socketId).emit('receiveBluffHand', p.hand));
        broadcastBluffState(tableId);
        io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `🃏 Deck dealt! Let the lying begin.` });
    });

    socket.on('playBluffCards', (data) => {
        const { tableId, cardIndices, claimedRank } = data;
        let table = bluffRooms[tableId];
        let player = activePlayers.find(p => p.socketId === socket.id && p.game === 'bluff');
        
        if (!table || !player || table.stage !== 'playing') return;

        let playedCards = cardIndices.map(i => player.hand[i]);
        player.hand = player.hand.filter((_, i) => !cardIndices.includes(i));
        
        table.lastPlayedCards = playedCards;
        table.centerPile.push(...playedCards);
        table.lastPlayer = player.username;
        table.latestClaim = `${player.username} played ${playedCards.length} ${claimedRank}(s)`;
        ...
        table.timer = setTimeout(() => {
            if (table.stage === 'interrogation') {
                table.stage = 'playing';
                if (player.hand.length === 0) {
                    table.stage = 'waiting';
                    io.to(tableId).emit('receiveChat', { username: "SYSTEM", message: `🏆 ${player.username} is out of cards and WINS!` });
                } else {
                    let activeOnly = activePlayers.filter(p => p.tableId === tableId && p.game === 'bluff' && p.hand.length > 0);
                    table.turnIndex = (table.turnIndex + 1) % activeOnly.length;
                }
                broadcastBluffState(tableId);
            }
        }, 10000);
    ...
    socket.on('disconnect', () => {
        ...
        if (player) {
            ...
            activePlayers = activePlayers.filter(p => p.socketId !== socket.id);
            if (player.game === 'blackjack') { broadcastTableState(tableId); checkRoundEnd(tableId, getUserBalance, updateBalance); } 
